...
var require_node_stream_zip = __commonJS({
  "node_modules/.pnpm/node-stream-zip@1.15.0/node_modules/node-stream-zip/node_stream_zip.js"(exports2, module2) {
    var fs2 = require("fs");
    ...
    var StreamZip2 = function(config) {
      ...
      function extract(entry, outPath, callback) {
        that.stream(entry, (err, stm) => {
          if (err) {
            ...
            fs2.open(outPath, "w", (err2, fdFile) => {
              if (err2) {
              ...
              fsStm = fs2.createWriteStream(outPath, { fd: fdFile });
              fsStm.on("finish", () => {
      ...
      function createDirectories(baseDir, dirs, callback) {
        ...
        let dir = dirs.shift();
        dir = path2.join(baseDir, path2.join(...dir));
        fs2.mkdir(dir, { recursive: true }, (err) => {
      ...
      function extractFiles(baseDir, baseRelPath, files, callback, extractedCount) {
        ...
        const targetPath = path2.join(baseDir, file.name.replace(baseRelPath, ""));
      ...
      this.extract = function(entry, outPath, callback) {
        ...
        if (!entry || entry.isDirectory) {
          ...
          fs2.stat(outPath, (err, stat) => {
            if (stat && stat.isDirectory()) {
              extract(entry, path2.join(outPath, path2.basename(entry.name)), callback);
            } else {
    ...
    var EntryDataReaderStream = class extends stream.Readable {
      ...
      _read(n) {
        ...
        if (buffer.length) {
          fs2.read(this.fd, buffer, 0, buffer.length, this.offset + this.pos, this.readCallback);
        } else {
          this.push(null);
        }
      }
...
function request(url) {
  return new Promise((resolve, reject) => {
    const protocol = url.startsWith("https") ? import_node_https.default : import_node_http.default;
    const req = protocol.get(url);
    req.on("error", (error) => reject(error));
    req.on("response", (res) => {
      if (res.statusCode && res.headers.location && res.statusCode >= 300 && res.statusCode <= 399)
...
async function downloadPlugin(url, slug, saveFolder = "") {
  const pluginDataPath = saveFolder || LiteLoader.plugins.pluginStore.path.data;
  const body = (await request(url)).data;
  const cacheFilePath = import_node_path.default.join(pluginDataPath, `${slug}.zip`);
  import_node_fs.default.mkdirSync(pluginDataPath, { recursive: true });
  import_node_fs.default.writeFileSync(cacheFilePath, body);
  return cacheFilePath;
}
async function downloadFile(url, fileName, saveFolder = "") {
  const pluginDataPath = saveFolder || LiteLoader.plugins.pluginStore.path.data;
  const body = (await request(url)).data;
  const cacheFilePath = import_node_path.default.join(pluginDataPath, fileName);
  import_node_fs.default.mkdirSync(pluginDataPath, { recursive: true });
  import_node_fs.default.writeFileSync(cacheFilePath, body);
  return cacheFilePath;
}
async function installPlugin(cacheFilePath, slug) {
  const { plugins } = LiteLoader.path;
  const pluginPath = `${plugins}/${slug}`;
  try {
    import_node_fs.default.mkdirSync(pluginPath, { recursive: true });
    const zip = new import_node_stream_zip.default.async({ file: cacheFilePath });
    const entries = await zip.entries();
    const isFolder = !entries.hasOwnProperty("manifest.json");
    for (const entry of Object.values(entries)) {
      if (!entry.name.includes(".github")) {
        const pathname = `${pluginPath}/${isFolder ? entry.name.split("/").slice(1).join("/") : entry.name}`;
        if (entry.isDirectory) {
          import_node_fs.default.mkdirSync(pathname, { recursive: true });
          continue;
        }
        try {
          if (entry.isFile) {
          ...
          import_node_fs.default.mkdirSync(pathname.slice(0, pathname.lastIndexOf("/")), { recursive: true });
          await zip.extract(entry.name, pathname);
          continue;
        }
      }
    ...
    import_electron.dialog.showErrorBox("\u63D2\u4EF6\u5546\u5E97", error.stack || error.message);
    import_node_fs.default.rmSync(pluginPath, { recursive: true, force: true });
...
async function install(url, slug) {
  output(url);
  try {
    const cacheFilePath = await downloadPlugin(url, slug);
    return await installPlugin(cacheFilePath, slug);
  } catch (error) {
    import_electron.dialog.showErrorBox("\u63D2\u4EF6\u5546\u5E97", error.stack || error.message);
    return "\u4E0B\u8F7D\u5931\u8D25";
  }
}
async function uninstall(slug, updateMode = false) {
  const paths = LiteLoader.plugins[slug].path;
  if (!paths)
    return false;
  if (updateMode) {
    import_node_fs.default.rmSync(paths.plugin, { recursive: true, force: true });
    return true;
  }
  for (const [_, path2] of Object.entries(paths))
...
async function update(url, slug) {
  try {
    ...
    if (!await uninstall(slug, true))
      return "\u66F4\u65B0\u5931\u8D25";
    if (!(await installPlugin(cacheFilePath, slug) === "\u5B89\u88C5\u6210\u529F"))
      return "\u66F4\u65B0\u5931\u8D25";
    return "\u66F4\u65B0\u6210\u529F";
  } catch (error) {
    import_electron.dialog.showErrorBox("\u63D2\u4EF6\u5546\u5E97", error.stack || error.message);
...
function onLoad() {
  import_electron.ipcMain.handle(
    ...
    (event, ...message) => install(message[0], message[1])
  );
  import_electron.ipcMain.handle(
    "LiteLoader.pluginStore.downloadFile",
    async (event, ...message) => {
      try {
        await downloadFile(message[0], message[1], message[2]);
        return "\u4E0B\u8F7D\u6210\u529F";
      } catch (error) {
        import_electron.dialog.showErrorBox("\u63D2\u4EF6\u5546\u5E97", error.stack || error.message);
        return "\u4E0B\u8F7D\u5931\u8D25";
  ...
  import_electron.ipcMain.handle(
    ...
    (event, ...message) => uninstall(message[0], message[1])
  );
  import_electron.ipcMain.handle(
    "LiteLoader.pluginStore.update",
    (event, ...message) => update(message[0], message[1])
  );
  import_electron.ipcMain.handle(
  ...
  import_electron.ipcMain.on(
    "LiteLoader.pluginStore.openWeb",
    (event, url) => import_electron.shell.openExternal(url)
  );
  import_electron.ipcMain.on("LiteLoader.pluginStore.createWin", (event, message) => {
  ...
  import_electron.ipcMain.on("LiteLoader.pluginStore.createBrowserWindow", (_, oldStoreData) => {
    storeData = JSON.parse(oldStoreData);
    const preloadPath = process.env.NODE_ENV === "development" ? import_node_path.default.join(__dirname, "..", "preload", "index.js") : import_node_path.default.join(__dirname, "preload.js");
    const newWindow = new import_electron.BrowserWindow({
      // frame: false,
      transparent: false,
    ...
    newWindow.webContents.on("before-input-event", async (__, input) => {
    ...
    if (process.env.NODE_ENV === "development")
      newWindow.loadURL("http://localhost:5173/store/");
    else
      newWindow.loadFile(import_node_path.default.join(__dirname, "view/index.html"));
    ...
    if (!protocol.isProtocolHandled("local")) {
      protocol.handle("local", async (req) => {
        ...
        const filepath = import_node_path.default.normalize(pathname.slice(1));
        return import_electron.net.fetch(`file://${host}/${filepath}`);
      });
    }
    newWindow.on("closed", () => {
    });
  });
  import_electron.ipcMain.on("LiteLoader.pluginStore.getStoreData", (event) => {
    event.returnValue = storeData;
  });
  import_electron.ipcMain.on("LiteLoader.pluginStore.log", (event, level, ...args) => {
    const levelMap = { 0: "debug", 1: "log", 2: "info", 3: "warn", 4: "error" };
    const consoleMethod = levelMap[level] || "log";
    console[consoleMethod](`[!Renderer:Log:${event.sender.id}]`, ...args);
  });
}
